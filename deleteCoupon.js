...
function delCoupon(couponId, couponTitle) {
    return new Promise(resolve => {
        const options = {
            url: `https://api.m.jd.com/client.action?functionId=deleteCouponListApplet&appid=jd-cphdeveloper-m&body={\"couponinfolist\":\"${couponId}\"}&_=${Date.now()}&sceneval=2&g_login_type=1&callback=jsonpCBKC&g_ty=ls`,
            headers: {
        ...
        $.get(options, (err, resp, data) => {
            try {
                data = JSON.parse(data.match(new RegExp(/jsonpCBK.?\((.*);*/))[1].replace(")", ""));
                if (data.retcode === 0) {
...
function isJDCoupon(title) {
    if (title.indexOf('京造') > -1)
        return false
    else if (title.indexOf('每月权益') > -1)
        return true
    else if (title.indexOf('京东') > -1)
        return true
